<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="cart.aspx.cs" Inherits="agri.shop.cart" %>
...
<script>
    ...
    function loadCart() {
        $.ajax({
            ...
            success: function (response) {
                    ...
                    html += `<tr>
                        <td class="product-col">
                            <div class="product">
                                <figure class="product-media">
                                    <a href="#">
                                        <img src="../${item.ImageUrl}" alt="Product image" style="width: 60px; height: 60px; object-fit: cover;">
                                    </a>
                                </figure>
                        ...
                        <td class="quantity-col">
                            <div class="cart-product-quantity">
                                <input type="number" class="form-control" value="${item.Quantity}" min="1" data-id="${item.Id}" onchange="updateQuantity(this)" />
                            </div>
                        </td>
                        <td class="total-col">$${total.toFixed(2)}</td>
                        <td class="remove-col">
                            <button class="btn-remove" onclick="removeItem(${item.Id})"><i class="icon-close"></i></button>
                        </td>
                    </tr>`;
                });
                ...
                $('#total-amount').text(`$${subtotal.toFixed(2)}`); // Assuming no shipping charges for now
            },
            error: function (err) {
    ...
    function updateQuantity(input) {
        ...
        $.ajax({
            ...
            data: JSON.stringify({ productId: id, quantity: newQty }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function () {
                loadCart();
